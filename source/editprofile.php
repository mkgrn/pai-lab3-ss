    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($new_picture)) {
            if ((($new_picture_type == 'image/gif') || ($new_picture_type == 'image/jpeg') ||
                ...
                // Nowy plik jest nieprawidłowy, dlatego trzeba usunąć tymczasowy pliki ustawić flagę błędu.
                @unlink($_FILES['new_picture']['tmp_name']);
                $error = true;
        ...
        if (!$error) {
            if (
                ...
                echo '<p>Aktualizacja profilu zakończyła się sukcesem. Czy chcesz <a href="viewprofile.php"> '
                    . 'zobaczyć swój profil</a>?</p>';
                mysqli_close($dbc);
                exit();
            } else {
    ...
    <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        <fieldset>
            ...
            <select id="gender" name="gender">
                <option value="M" <?php if (!empty($gender) && $gender == 'M')
                                        echo 'selected = "selected"'; ?>>Mężczyzna</option>
                <option value="K" <?php if (!empty($gender) && $gender == 'K')
                                        echo 'selected = "selected"'; ?>>Kobieta</option>
            </select><br />
